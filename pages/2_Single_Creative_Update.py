...
st.set_page_config(
    page_title="Single Creative Updater",
    layout="wide"
)

st.title("Single Creative Updater")
...
def get_creds():
    ...
    if os.path.exists('token.json'):
        try:
            creds = Credentials.from_authorized_user_file('token.json', ['https://www.googleapis.com/auth/display-video'])
...
def load_existing_trackers():
    ...
    try:
        with st.spinner("Loading creative trackers..."):
            ...
            df['new_url'] = ""
            st.session_state.tracker_df_single = df
...
def update_creative():
    if "tracker_table_single" not in st.session_state or st.session_state.tracker_df_single is None:
        st.error("No tracker data to update. Please load trackers first.")
        return
    try:
        with st.spinner("Applying edits and updating creative..."):
            edited_df = st.session_state.tracker_df_single.copy()
            delta = st.session_state.tracker_table_single
            ...
            # Process the reconstructed DataFrame
            final_trackers = []
...
if creds:
    if "tracker_df_single" not in st.session_state:
        st.session_state.tracker_df_single = None
    ...
    if st.session_state.tracker_df_single is not None:
        ...
        st.data_editor(
            st.session_state.tracker_df_single,
            num_rows="dynamic",
